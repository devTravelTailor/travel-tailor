...
const getTitle = (item) =>
  item?.title || item?.name || item?.heading || 'Coming soon';
...
const getLocation = (item) =>
  item?.place || item?.destination || item?.location || item?.country;
  ...
  const renderHeading = (text) => {
    ...
    return (
      <h2 className='text-3xl md:text-4xl font-semibold text-center text-gray-900'>
        <span className='text-[#ff5b06] text-4xl md:text-5xl mr-2 font-handwriting'>
          {first}
        </span>
        {rest ? `  ${rest}` : ''}
      </h2>
